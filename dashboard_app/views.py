...
def all_bookings(request):
    ...
    query="select bookingid,customerid,laundry_order_bookingtbl.address,laundry_order_bookingtbl.city,laundry_order_bookingtbl.pincode,laundry_order_bookingtbl.landmark,time_at,booking_status,profile_img,mobile_no,usrname,device_token,name,order_id_generated from vff.laundry_delivery_boytbl,vff.usertbl,vff.laundry_customertbl,vff.laundry_order_bookingtbl where laundry_delivery_boytbl.delivery_boy_id=laundry_order_bookingtbl.delivery_boy_id  and laundry_customertbl.consmrid=laundry_order_bookingtbl.customerid and laundry_customertbl.usrid=usertbl.usrid and  laundry_order_bookingtbl.delivery_boy_id!='-1' and booking_status!='NA'  and branch_id='"+str(branch_id)+"' order by bookingid desc"
    query_result = execute_raw_query(query)
    data = []    
    if not query_result == 500:
        for row in query_result:
...
def check_notifications(request):
    ...
    query="select bookingid,laundry_order_bookingtbl.address,laundry_order_bookingtbl.city,laundry_order_bookingtbl.pincode,laundry_order_bookingtbl.landmark,time_at,customer_name,profile_img from vff.laundry_customertbl,vff.laundry_order_bookingtbl,vff.usertbl where laundry_order_bookingtbl.customerid=laundry_customertbl.consmrid and laundry_customertbl.usrid=usertbl.usrid and booking_status='NA'  and branch_id='"+str(branch_id)+"' and booking_status='NA' and delivery_boy_id='-1' order by bookingid desc" #and booking_date='"+str(today_date)+"'
...
def add_items_to_cart(request):
    ...
    if request.method == "POST":
        ...
        if key_pair == 2:
            ...
            cat_id = jdict['cat_id']
            customer_id = jdict['customer_id']
            item_width_per_sqft_prv = jdict['item_width_per_sqft']
            item_height_per_sqft_prv = jdict['item_height_per_sqft']
                ...
                with connection.cursor() as cursor:
                    for item in all_data:
                        ...
                        if type_of == "Sqft":
                            ...
                            item_width_per_sqft = float(item_width_per_sqft_prv)
                            item_height_per_sqft = float(item_height_per_sqft_prv)
